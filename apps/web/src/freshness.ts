 ...
 * マニフェストの印が接続の番号まで見えるところに置かれ、検証を接続のたびに
 * やり直させる結び付きが型からは見えないまま入り込む。
 *
 * 環境変数の入力値そのものは畳み込まない（§2.4）。書き換えの回数だけで「変わった」は言えるので、
 * 値を持ち回す必要が無い。
 */
...
/**
 * 接続の印はフォームの入力値ではなく、確立した接続の番号に付ける（WU-36）。入力値に
 * 付けると、切り替えのモーダルに1文字打った時点で今の接続と計画が無効になる。
 */
