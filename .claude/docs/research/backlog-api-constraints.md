...
課題種別の色パレットは、1スペースの 70 プロジェクト・674 課題種別を走査して
**色の種類がちょうど 10 に収束する**ことを確認した（2026-09-19 実測）。
既定4色はいずれもこの10色に含まれる。
...
| `issues/count` が数える範囲 | **完了済みを含む全件。** ある14,711件のプロジェクトで、ステータス別（3,822 / 5 / 4 / 10,880）の合計と一致した |
...
| 項目 | 備考 | 確認に必要なもの |
| --- | --- | --- |
| `POST /projects` の必須パラメータ | `settings` を省略したとき、ツールが既定値を補って送るべきキーが決まらない | **スペース管理者権限** |
...
上3件は**実装の骨格に影響する**が、いずれも書き込みを伴うため
スペース管理者権限を持つ検証用スペースが要る。
æ¤è¨¼ã«ä½¿ã£ãã¹ãã¼ã¹ での実行者は `roleType: 2`（スペース管理者ではない）なので、
この空間では確認できない。
