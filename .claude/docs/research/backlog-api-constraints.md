...
最終更新: 2026-09-18 / 出典: https://developer.nulab.com/docs/backlog/
...
| 項目 | 備考 |
| --- | --- |
| カスタム属性のプランによる利用可否 | 利用可能なプランでしか確認していない。プラン差を扱うなら要確認 |
| チーム機能のプラン制限 | 同上 |
| `useDevAttributes: false` 時のマイルストーン・版の扱い | 整合性検証を厳密にするなら要確認 |

いずれもプラン差に関わるもので、**実装の骨格には影響しない**。
プラン差を吸収する必要が出た時点で確認すればよい。
