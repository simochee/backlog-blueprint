...
- **`--auto-approve` の無い `apply --output json`**。[CL-10](cli-and-web-ui.md#cl-10-apply---output-json-は---auto-approve-を必須にする)
  により、マニフェストを読む前に引数の誤りとして止まり、stdout には何も書かない。状態ではなく引数だけで決まる誤りで、
  CI に組み込んだ最初の1回で必ず表に出るため。CL-6 で止まる場面はこれで `--output text` にしか残らない
