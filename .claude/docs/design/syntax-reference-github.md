...
> **既定ステータスには `oldname` を使えない。** 実 API で確認したところ、
> 既定ステータスは名前も色も変更できず、削除もできない。
> `- name: Done / oldname: 完了` のような書き換えは成立しないため、
> マニフェストには既定4つをそのまま書く（V-A6 / V-A6a）。
> `oldname` が効くのは課題種別・カテゴリー・マイルストーン・カスタム属性、
> およびカスタムステータスに限られる。
