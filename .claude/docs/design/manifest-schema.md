...
### S-1: 配列キーの省略は「空配列」であって「管理対象外」ではない
...
### S-2: `issueTypes` / `statuses` を JSON Schema の `required` に入れない
...
配列（S-1）と違い、`settings` の各キーは削除の概念を持たないスカラーである。
書かれたキーだけを `POST` / `PATCH` のパラメータに載せ、書かれなかったキーは送らない。

**既存プロジェクトの更新**では、送らなければ現状が保たれる。
**新規作成**では、`POST /api/v2/projects` が必須としているパラメータだけは
ツールが既定値を補って送る必要がある。

> `要検証` — `POST /api/v2/projects` の必須パラメータがどれかは
> [API 制約](../research/backlog-api-constraints.md#プロジェクト基本設定) に記載が無い。
> 実測で確定させ、必須なものについてはツール側が持つ既定値を本節に表で記す。
> 確定するまでは「全キーを明示的に書いた雛形」を配布して回避する。
...
| --- | --- | --- | --- |
| `name` | string | **必須** | `minLength: 1` |
| `color` | string | **必須** | `^#[0-9a-fA-F]{6}$` |
| `templateSummary` | string | — | |
| `templateDescription` | string | — | |
| `oldname` | string | — | `minLength: 1` |

### S-4: 課題種別の色にパレット制約を課さない

ステータスの色は10色のパレット外が拒否されることを実測済みだが
（[API 制約](../research/backlog-api-constraints.md#ステータス)）、
**課題種別の色に同じ制約があるかは調査されていない**。

未検証の制約を JSON Schema に書くと、実際には通る色を赤くしてしまう。
誤検出はツールへの信頼を直接削るので、形式（6桁の16進）だけを検証し、
値の妥当性は API に判定させる。拒否されたらそのエラーをそのまま見せる
（[要件定義 §9](../requirements/requirements-definition.md#9-残る未検証事項) と同じ方針）。

> `要検証` — 課題種別の色パレット。固定パレットであることが確認できたら
> ステータスと同じく enum に変える。
...
### S-5: 既定ステータスの判定を JSON Schema でやらない
...
### S-6: カスタム属性の型は数値 `typeId` を受け付けない
...
### S-7: イベント名は `enum` ではなく `const` の `oneOf` で書く
...
| V-A6 / V-A6a / V-A8 / V-A14 ステータス関連 | 既定かどうかの判定に ID（＝スナップショット）が要る（S-5） |
...
| S-1 | キー省略 = 空配列 | 省略 = 管理対象外。「書き忘れ」と「意図的に管理しない」が区別できなくなる |
| S-4 | 課題種別の色はパターンのみ | ステータスと同じパレット enum。未検証の制約で誤検出を出す |
| S-5 | 既定ステータス判定を Schema でやらない | 既定名を enum に持つ。言語設定が違うスペースで誤検出 |
| S-6 | カスタム属性の型は名前のみ | 数値も許す。前方互換の必要が無いところに書き方を2つ作る |
| S-7 | イベント名は `const` の `oneOf` | `enum`。値ごとの説明を補完に出せない |
