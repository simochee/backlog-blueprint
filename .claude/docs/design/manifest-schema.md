...
| E-4 | 展開された値は `Secret` として扱い、出力時に常にマスクする。**ただし同定名は除く**（E-7） | FR-3.6 / NFR-3。マスク対象を「Webhook URL だけ」のように列挙すると、対象の追加漏れが漏洩になる |
| E-5 | Yaml に直接書かれた値はマスクしない | 既にリポジトリに平文で入っている。マスクしても秘匿性は増えず、plan が読みにくくなるだけ |

### E-7: 同定名は `Secret` にできない
...
しかも「秘匿すべき値をリソース名にした」場合に限る。

黙って漏らすのではなく V-A25 で警告するのは、**利用者が気づける形にするため**である。
エラーにしない理由は、プロジェクト名やカテゴリー名を環境ごとに変える運用が普通にありうるため。

### E-6: マニフェストの型は `Secret` を持たず、展開した経路を別に持つ

| ID | 決定 |
| --- | --- |
| E-6 | S2 は展開後の**素の文字列**をマニフェストに残し、`${ENV}` 由来の値がどの `path` にあったかを集合として別に返す。`Secret` で包むのは `Action` を組み立てるとき。**その集合は `PlanContext.isSecret(path)` として `plan()` に渡す**（[core のデータモデル §6](core-reconciler.md#6-reconciler-インターフェース)） |
...
**包む側は `plan()` である。** 各 reconciler は「この param がマニフェストのどの `path` 由来か」を
知っている唯一の場所なので、`PlanContext.isSecret(path)` を引いて包む。
`hookUrl` だけを特別扱いして包む実装は採らない。それは E-4 が
「マスク対象を Webhook URL だけのように列挙すると、対象の追加漏れが漏洩になる」として
明示的に退けた形そのものである。`templateDescription` も `description` も `${ENV}` を書ける（E-1）。
...
| E-7 | 同定名は包まず V-A25 で警告 | 同定名も包む。解決表のキーと `Action.id` が引けなくなり、中断レポートが「どこまで適用されたか」を表せない |
