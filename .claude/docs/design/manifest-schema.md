...
> 確認にはスペース管理者権限が要り、現在の検証環境（`roleType: 2`）では確認できていない。
...
### K-4: 課題種別の色は enum にするが、検証は警告に留める

| 決定 | 内容 |
| --- | --- |
| K-4 | 課題種別の `color` は上記10色の enum とする。ただしパレット外の値は**警告**（V-A8 と違い、エラーにしない） |

70 プロジェクト・674 課題種別を走査して色の種類がちょうど10に収束することは確認したが、
**パレット外の色を API が拒否するかは確認できていない**（作成に権限が要る）。

ステータス（V-A8）をエラーにできるのは、パレット外が `error.unknown : color` で
拒否されることを実測しているから。同じ根拠が無いものを同じ強さで止めるのは、
[K-5](#k-5-既定ステータスの判定を-json-schema-でやらない) で誤検出を避けた判断と一貫しない。

enum にすることでエディタ補完には10色が出る。値の最終的な妥当性は API に判定させ、
拒否されたらそのエラーをそのまま見せる
（[要件定義 §9](../requirements/requirements-definition.md#9-残る未検証事項) と同じ方針）。

> `要検証` — パレット外の色の拒否挙動。拒否されることが確認できたら
> ステータスと同じくエラーに格上げする。
...
| K-4 | 課題種別の色はパターンのみ | ステータスと同じパレット enum。未検証の制約で誤検出を出す |
