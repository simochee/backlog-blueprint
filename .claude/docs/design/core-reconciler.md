...
そこで**名前を使わない**。`refresh` が既定4件を**返ってきた順に位置キーでも解決表へ登録**し、
`plan()` はマニフェストの課題種別を先頭から枠に割り当てる。

| マニフェストの件数 N | 生成する Action |
| --- | --- |
| 先頭から `min(N, 4)` 件 | 枠を引き継ぐ。`op: 'create'`、`request.method: 'PATCH'`、`target` は枠の位置キー |
| N > 4 の残り | 通常の `create`（`POST`） |
| N < 4 で余った枠 | `delete`（振替先は先頭の枠） |

**枠の引き継ぎを「リネーム」として表さない。** `op` は `create` とし、`changes[].before` は
すべて `null`、`notes` も付けない。PO-1 が `oldname` のリネームを `~` で表すと決めたのは、
**利用者が `oldname` を書いた効果を plan 上で見えるようにする**ためである。枠の引き継ぎは
利用者が書いたものではなくツールが新規プロジェクトで勝手に行う最適化なので、この理由が当てはまらない。
利用者から見れば、何も無いところに課題種別を宣言しただけである。
...
ID は言語に依存しないので、枠として引き継げば表そのものが不要になる。

**採らなかった案: 新規プロジェクトでは既定を全削除して作り直す。** 名前は不要になるが、
L-2 / L-6 が数えている節約（既定を使い回せば削除＋作成の2リクエストが1リクエストになる）が
新規プロジェクトで丸ごと消える。
...
| 決定 | 内容 |
| --- | --- |
| RF-1 | プロジェクトが存在しない場合に限り、`project/create` の直後に `op: 'refresh'` の Action を1件挿入する。`/projects/:key/issueTypes` と `/projects/:key/statuses` を取得し、解決表に流し込む。**課題種別は名前キーに加えて、返ってきた順の位置キーでも登録する**（§4.1） |
