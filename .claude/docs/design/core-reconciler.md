...
type ResourceKind =
  ...
  | 'webhook'
  | 'issueTypeSlot'   // 解決表の中だけで使う。Action.kind には現れない（§4.1）
...
`Reconciler.kind`（§6）は1つしか持てないので、access の reconciler が名乗る `kind` は**代表値にすぎない**。
実行も描画も `Action.kind` を見る。
...
| --- | --- |
| 課題種別 | 既定4件。**名前も ID も未知**（下記）。件数だけが分かる |
| ステータス | 既定4件。**ID は 1〜4 の既知の固定値** |
| その他すべて | 空 |

#### 新規プロジェクトの既定課題種別は、名前ではなく枠として引き継ぐ

既定の表示名はスペースの言語設定で変わる。既存プロジェクトは `GET` が実名を返すので
問題にならないが、**新規プロジェクトはまだ存在しないので名前を取得できない**。
課題種別は ID がプロジェクト固有なので、§6.2 の照合は名前で行われる。
名前を推測して計画を組むと、`{$ref:issueType:タスク}` を指す削除 Action に対して
`refresh` が登録するのは `issueType:Task` になり、**適用の実行時に未解決参照で中断する**。

そこで**名前を推測しない**。`plan()` はマニフェストの課題種別を先頭から既定の枠へ割り当て、
`refresh` は取得した既定を**返ってきた順に、割り当て先のマニフェスト名で解決表へ登録する**。

| マニフェストの件数 N | 生成する Action | 解決表への登録 |
| --- | --- | --- |
...
最終的な名前で指せる。中間の識別子を作ると、それが利用者の付けた名前と同じ名前空間に入り、
`#1` という名前の課題種別を書かれたときに**黙って別の枠を書き換える**。

余った枠だけは対応する名前が無いので位置で指すしかない。そこは `issueTypeSlot` という
別の `ResourceKind`（§2.1）に逃がし、利用者の名前と交わらないようにする。

##### 利用者は初期状態にあるものを認識しなくてよい

これがこの設計の根にある原則である。利用者が書くのは**あるべき姿**であって、
Backlog が最初に置いた既定リソースは利用者の関心事ではない。したがって:

- 枠の引き継ぎは `create`（`+`）で描く。「既定の何かを引き継いだ」と伝えない
- `changes[].before` は `null`、`notes` も付けない。引き継ぎ元を示さない
- 余った枠の削除は粛々と行う。マニフェストに書いていないものは消える（要件定義 §2.2）という
  宣言的な約束と、利用者の意図が一致している

**採らなかった案: 枠に `#0` のような中間の名前を与える。** 上記の衝突が静かに起きる。

**採らなかった案: スペースの言語ごとに既定名の表を持つ。** `GET /api/v2/space` の `lang` で
組を選べるが、Backlog が対応言語を増やすたびに表の追随が要り、表に無い言語では計画を組めない。

**採らなかった案: 新規プロジェクトでは既定を全削除して作り直す。** 名前は不要になるが、
L-2 / L-6 が数えている節約が新規プロジェクトで丸ごと消える。

`op: 'create'` なのに `request.method` が `PATCH` で `target` が付くのは、
`request` が実際に飛ぶリクエストを正確に表す（PO-3）以上そうなる。
`op` は「宣言したリソースがどうなるか」を表し、HTTP のメソッドと一致する必要はない（`reorder` も同様）。

べき等性（NFR-4）は保たれる。2回目は `GET` が実名を返すので §6.2 の名前照合に当たり、全件 `noop` になる。
AC-2 も満たす。既定はマニフェストの種別に化けるか削除されるかのどちらかで、残らない。
...
| 決定 | 内容 |
| --- | --- |
| RF-1 | プロジェクトが存在しない場合に限り、`project/create` の直後に `op: 'refresh'` の Action を1件挿入する。`/projects/:key/issueTypes` と `/projects/:key/statuses` を取得し、解決表に流し込む。**課題種別は、引き継ぐ枠をマニフェストの名前で、余った枠を `issueTypeSlot:<位置>` で登録する**（§4.1） |
...
**チーム経由のみの参加者には管理者を付与できない**（実測）。`No such project member` が返る。
したがって「個人参加していない管理者には必ず個人参加の Action を出す」は
安全側の措置ではなく**必須**である。A-3 と同じ扱いになる。
...
| `actionFailed` | API エラー、または HTTP の応答を得られなかった失敗（タイムアウトを含む。X-7）。直後に `aborted` |
...
```ts
type HttpFailure = {
  status?: number                  // HTTP の応答を得られなかった失敗では無い（plan の出力仕様 §3.3 / PO-5）
...
Backlog から応答が返らないまま TCP 接続が止まると、待ち時間を持たない送信は
`plan` / `apply` / `export` を何も言わずに止め続ける。CLI も Web UI も同じように止まる。

| 決定 | 内容 | 理由 |
| --- | --- | --- |
| X-5 | 1リクエストの応答を待つのは**60秒まで**。GET も更新系も同じ値で、`GET /rateLimit` も画像の取得も含む。計る範囲は `fetch` を呼んでから本文を読み終えるまで。超えたら接続を中断し、`status` の無い失敗（§7.0）として投げる | 値は CLI の一般的な既定値にそろえる。AWS CLI の `--cli-connect-timeout` / `--cli-read-timeout` の既定が60秒で、Backlog の応答時間を実測して決めるものではない。読みと書きで値を分ける根拠も無い。本文の途中で止まる接続もあるので、ヘッダの到着では打ち切らない |
| X-6 | タイムアウトは**再試行しない**。失敗として扱い、FR-4.3 のとおり止める | 更新系は届いていた場合に二重に適用されうる。GET は二重にしても安全だが、止まった接続がすぐに戻る根拠は無く、X-4 のように待つべき時刻も分からない |
| X-7 | 更新系のタイムアウトは「**適用されなかった**」とは言わない。エラー内容に「適用済みかもしれない」旨を載せる | 送信は済んでいて、応答だけが返らなかった可能性がある。無いと言い切ると、利用者が手で作り直して二重にする |

**置き場所は送信層（`packages/backlog-client`）。** CLI と Web UI は同じクライアントを使うので、
待ち時間も同じになる（NFR-6）。core の Executor に置く案は採らない。core の `get` / `send` は
`Promise` を返すだけで中断の手段を持たず、core で待ち時間を競わせても送信は止まらず接続が残る。
加えて、止めた後に待ちを片付ける `clearTimeout` を core の面（NFR-5）に足すことになる。
送信層もブラウザで動くので、足すのは Node とブラウザの両方にある `AbortController` と
`clearTimeout` だけにする。

**backlog-js の `timeout` 設定は使わない。** 0.20.1 はこの値を `fetch` の `init.timeout` に
入れるだけで、これは node-fetch v2 の独自拡張である。Node の組み込み `fetch` もブラウザの
`fetch` も無視するので、渡しても何も起きない。

**60秒は X-1 の間隔にも X-4 の待機にも数えない。** どちらもリクエストの外にある待ちで、
コマンド全体の上限ではなくリクエスト1件ごとの上限である。設定可能にもしない。
CLI と Web UI の両方に入力の経路が要るうえ、値を選ぶ材料は利用者の側にも無い。

#### タイムアウトの現れ方

| 場面 | 現れ方 |
| --- | --- |
| S5（`GET /users/myself`） | V-B1 の診断。本文にタイムアウトが載る |
| V-B3 の課題件数 | V-B3 の診断。確認できなかった扱いで止まる（VP-5） |
| それ以外の read（`plan` / `apply` の計画 / `export`） | コマンドが止まり、送信層の失敗としてそのまま出る。`status` は無い |
| `apply` の `refresh` | GET なので X-7 の但し書きは付かない。中断レポートの形は他の失敗と同じ |
| `apply` の更新系 | 中断レポート（FR-4.4）で「失敗した操作」に置き、「未適用の操作」には入れない。エラー内容は2件で、応答が無かったことと、適用済みかもしれないこと（X-7） |

更新系のタイムアウトから再開するのも、他の中断と同じく同じマニフェストでの `apply` のやり直しで
足りる（§7.2 / NFR-4）。read からやり直すので、実は適用されていた変更は `noop` になる。
X-7 が言うのは「分からない」までで、分かる手段は次の `plan` が持っている。
...
| X-5 | 待ち時間は送信層が持つ | core の Executor で競わせる。送信が止まらず接続が残り、`clearTimeout` が core の面に増える。backlog-js の `timeout` は組み込みの `fetch` に無視される |
| X-6 | タイムアウトは再試行しない | GET だけ再試行する。止まった接続がすぐ戻る根拠が無く、待つべき時刻も分からない |
| X-7 | 更新系のタイムアウトは適用済みかもしれないと書く | 失敗として「未適用」と扱う。届いていた変更を利用者が作り直して二重にする |
