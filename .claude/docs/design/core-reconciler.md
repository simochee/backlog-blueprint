...
#### RD-1: スペースのチーム一覧は全ページを辿る

`/teams` はページング付きの一覧である（[API 制約](../research/backlog-api-constraints.md#スペースのチーム一覧のページング)）。
**`count=100` を付け、`offset` を 0 から 100 ずつ進め、返った件数が 100 未満になるまで取る。**
1回で済ませると1ページを超えたぶんが黙って欠け、V-B5 が存在するチームを「無い」と言う。

- `count` はリファレンスの上限にそろえる。省いたときの件数は `要検証` なので頼らない
- ページを辿る処理は core に1つだけ置き、Web UI の Teams ペイン（WU-22）も同じものを使う。
  ページの大きさと終わりの判定が2箇所に分かれると、片方だけが欠ける
- ページは直列に取る。次のページが要るかは、前のページの件数を見るまで分からない
...
| RD-1 | `/teams` は `count=100` で全ページを辿る | 引数なしで1回だけ取る。1ページを超えたチームが黙って欠け、V-B5 が誤ったエラーを出す |
